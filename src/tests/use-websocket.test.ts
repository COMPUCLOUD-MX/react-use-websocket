...
test("lastJsonMessage updates with a json object when websocket receives a message", async () => {
  ...
  const { result } = renderHook(() =>
    useWebSocket<{ name: string }>(URL, options),
  );
  await server.connected;
...
test.each([false, true])(
  "Options#heartbeat, if provided, sends a message to the server at the specified interval and works when share is %s",
  async (shareOption) => {
    options.heartbeat = {
      message: "ping",
      timeout: 10000,
...
test.each([false, true])(
  "Options#heartbeat, if provided, close websocket if no message is received from server within specified timeout and works when share is %s",
  async (shareOption) => {
    options.heartbeat = {
      message: "ping",
      timeout: 1000,
...
test.each([false, true])(
  "Options#heartbeat, if provided, do not close websocket if a message is received from server within specified timeout and works when share is %s",
  async (shareOption) => {
    options.heartbeat = {
      message: "ping",
      timeout: 1000,
...
test.each([false, true])(
  "Options#heartbeat, if provided, lastMessage is updated if server message does not matches the returnMessage property of heartbeatOptions and works when share is %s",
  async (shareOption) => {
    options.heartbeat = {
      message: "ping",
      returnMessage: "pong",
...
test.each([false, true])(
  "Options#heartbeat, can handle case when interval is very close to timeout",
  async (shareOption) => {
    options.heartbeat = {
      message: "ping",
      returnMessage: "pong",
